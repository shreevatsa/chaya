...
- **Annotation Management**: 
  ...
  - Hover highlighting for easy identification
  - Individual delete buttons and bulk "Clear All" option
...
### Viewer (`viewer.html`)
- *Planned*: Display PDFs with existing annotations (not yet implemented)
...
- **Delete**: Use the red 'X' button in the annotation list, or "Clear All" for bulk deletion
...
- **Hover Preview**: Hover over list items to highlight the corresponding annotation
...
### Code Organization
The codebase is currently organized as a single module in `src/annotator.ts` with the following main sections:

1. **PDF Rendering** (`renderPage`, PDF.js integration)
2. **Annotation Creation** (`setupAnnotationDrawing`, mouse event handling)  
3. **Interactive Editing** (`makeAnnotationInteractive`, resize/drag functionality)
4. **Annotation Management** (`updateAnnotationList`, sidebar functionality)
5. **Serialization** (`loadAnnotationsFromJson`, save functionality)
6. **Coordinate System** (fractional ↔ pixel coordinate conversion)
